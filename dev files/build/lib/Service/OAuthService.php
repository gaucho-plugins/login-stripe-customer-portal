...
class OAuthService extends \LSCP\Stripe\Service\AbstractService
{
    /**
    ...
    public function authorizeUrl($params = null, $opts = null)
    {
        ...
        $query = \LSCP\Stripe\Util\Util::encodeParameters($params);
    ...
    private function _parseOpts($opts)
    {
        if (\is_array($opts)) {
            if (\array_key_exists('connect_base', $opts)) {
                ...
                throw new \LSCP\Stripe\Exception\InvalidArgumentException('Use `api_base`, not `connect_base`');
            }
        }
        return \LSCP\Stripe\Util\RequestOptions::parse($opts);
    }
    /**
