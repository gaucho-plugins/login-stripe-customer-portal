...
abstract class Util
{
    ...
    {
        $types = 'v1' === $apiMode ? \LSCP\Stripe\Util\ObjectTypes::mapping : \LSCP\Stripe\Util\ObjectTypes::v2Mapping;
        if (self::isList($resp)) {
            $mapped = [];
            foreach ($resp as $i) {
        ...
        if (\is_array($resp)) {
            if (isset($resp['object']) && \is_string($resp['object']) && isset($types[$resp['object']])) {
                ...
                $class = \LSCP\Stripe\V2\Collection::class;
            } else {
                $class = \LSCP\Stripe\StripeObject::class;
            }
        ...
        foreach ($properties as $key => $property) {
            if (\array_key_exists($property->getName(), $json)) {
                if ('related_object' === $property->getName()) {
                    $related_object = new \LSCP\Stripe\RelatedObject();
    ...
    {
        if ($h instanceof \LSCP\Stripe\ApiResource) {
            return $h->id;
        }
        if (static::isList($h)) {
            $results = [];
